...
export async function runDashboardTool(name: string, input: Record<string, unknown>) {
  ...
  try {
    ...
    if (name === 'normalize_vnr' || name === 'pmb_normalize_vnr') {
      ...
      output = await runTicketAddressVerify(coerceTicketAddressVerifyInput(input));
      ...
      output = await runSafeGetCustomerByInsuranceNumber(coerceSafeInsuranceLookupInput(input));
