...
import { LEAPING_MCP_TOOLS } from '../tools/mcp-leaping-tool-catalog.js';
import { runLeapingMcpTool } from '../tools/mcp-leaping-tool-runner.js';
import { logCall } from '../db.js';

export const mcpRouter = Router();

// ── Leaping-visible tool catalogue (internal tools use dashboard / REST API) ─
...
mcpRouter.post('/sse', express.json(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'tools/list': {
        result = { tools: LEAPING_MCP_TOOLS };
        break;
      }
      ...
      case 'tools/call': {
        ...
        result = await runLeapingMcpTool(p.name, p.arguments ?? {});
        break;
      }
