  ...
  try {
    ...
    if (name === 'normalize_vnr' || name === 'pmb_normalize_vnr') {
      ...
      output = runVerificationMethodRouter(
        coerceVerificationMethodRouterInput(input as Record<string, unknown>)
      );
...
const LEAPING_FUNCTIONS = [
  {
    ...
    method: 'GET',
    url: 'https://pflegemittelbox.de/api_leapingai/kunde_vnr.php?token=…&insurance_number=…',
    parameters: ['insurance_number'],
    notes: 'VNR lookup via Marie PHP API. Prefer pmb_safe_get_customer_by_insurance_number MCP tool to avoid exposing full CRM payloads.',
    safe: true,
  ...
  },
  {
    ...
    type: 'Leaping HTTP Function',
    method: 'GET',
    url: 'https://pflegemittelbox.de/api_leapingai/kunde_plzb.php?token=…&plz=…&hnr=…&bday=…',
    parameters: ['plz', 'hnr', 'bday'],
    notes: 'Address lookup via Marie PHP API. Prefer pmb_safe_get_customer_by_plz_geb MCP tool to avoid exposing full CRM payloads.',
    safe: true,
    productionChanging: false,
  },
  {
    ...
    type: 'MCP Tool',
    method: 'POST',
    url: '(MCP server proxies GET kunde_vnr.php)',
    parameters: ['insurance_number'],
    notes: 'Safe VNR lookup — returns only { found, id?, birthday_present? }.',
    safe: true,
    productionChanging: false,
  },
  {
    ...
    type: 'MCP Tool',
    method: 'POST',
    url: '(MCP server proxies GET kunde_plzb.php)',
    parameters: ['plz', 'hnr', 'bday'],
    notes: 'Safe address lookup — returns only { found, id?, birthday_present? }.',
    safe: true,
    productionChanging: false,
  },
  {
